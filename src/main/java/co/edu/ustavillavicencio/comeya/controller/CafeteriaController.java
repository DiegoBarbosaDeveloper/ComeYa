...
public class CafeteriaController {
    ...
    @PostMapping
    public ResponseEntity<CafeteriaResponse> create(@Valid @RequestBody CafeteriaRequest req) {
        return ResponseEntity.ok(cafeteriaService.create(req));
    }

    @GetMapping
    public ResponseEntity<Page<CafeteriaResponse>> list(@RequestParam(required = false) String q, Pageable pageable) {
        return ResponseEntity.ok(cafeteriaService.search(q, pageable));
    }
